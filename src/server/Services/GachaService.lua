...
function GachaService:PullGacha(player: Player, pullType: "Single" | "Multi"): { ImaginarySlime }
	...
	for i = 1, count do
		...
		print("[GachaService] " .. player.Name .. " pulled: " .. slime.Name .. " (" .. slime.Rarity .. ")")
	end
...
function GachaService:GetPlayerGachaCollection(player: Player): { ImaginarySlime }
	if not playerGachaInventory[player] then
		return {}
	end
...
function GachaService:GetImaginarySlime(player: Player, instanceId: string): ImaginarySlime?
	if not playerGachaInventory[player] then
		return nil
	end
...
function GachaService:LoadPlayerGachaData(player: Player, data: { ImaginarySlime })
	if data then
		playerGachaInventory[player] = {}
...
-- Get save data
function GachaService:GetSaveData(player: Player): { ImaginarySlime }
...
function GachaService:GetRateInfo(): { [string]: number }
...
Players.PlayerAdded:Connect(function(player)
	playerGachaInventory[player] = {}
	print("[GachaService] Initialized for " .. player.Name)
end)
