...
local function getMemoryUsage(): number
    local success, result = pcall(function()
        return gcinfo() -- Roblox-compatible memory check (returns KB)
    end)
...
local function checkPerformanceWarnings()
    ...
    local memoryMB = metrics.MemoryUsage / 1024 -- gcinfo() returns KB, convert to MB
    if memoryMB > MEMORY_WARNING_THRESHOLD then
        table.insert(warnings, "High memory usage: " .. math.floor(memoryMB) .. "MB")
    end
