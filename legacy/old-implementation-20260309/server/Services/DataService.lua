...
function DataService:_LoadData(player: Player)
    ...
    local WordExcavatorService = Knit.GetService("WordExcavatorService")
    if WordExcavatorService and data.GachaCollection then
        WordExcavatorService:LoadPlayerGachaData(player, data.GachaCollection)
    end
