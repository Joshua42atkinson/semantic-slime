...
local SynonymDatabase = require(game:GetService("ReplicatedStorage"):WaitForChild("Shared"):WaitForChild("SynonymDatabase"))
...
function BattleUI.ShowBattle(battleData: any)
	...
	-- Prompt & Semantic Input Area
	local inputContainer = Instance.new("Frame")
	inputContainer.Name = "InputContainer"
	inputContainer.Size = UDim2.fromOffset(500, 60)
	inputContainer.Position = UDim2.fromScale(0.5, 0.72)
	inputContainer.AnchorPoint = Vector2.new(0.5, 0.5)
	inputContainer.BackgroundColor3 = COLORS.CardBg
	inputContainer.Parent = battleScreen
	
	local inputCorner = Instance.new("UICorner")
	inputCorner.CornerRadius = UDim.new(0, 12)
	inputCorner.Parent = inputContainer
	
	local inputPrompt = Instance.new("TextLabel")
	inputPrompt.Name = "Prompt"
	inputPrompt.Size = UDim2.fromScale(1, 0.4)
	inputPrompt.Position = UDim2.fromScale(0, -0.6)
	inputPrompt.BackgroundTransparency = 1
	inputPrompt.TextColor3 = COLORS.Accent
	inputPrompt.Font = Enum.Font.GothamBold
	inputPrompt.TextSize = 16
	inputPrompt.Text = "Type a SYNONYM of your slime to ATTACK!"
	inputPrompt.Parent = inputContainer
	
	local damagePreview = Instance.new("TextLabel")
	damagePreview.Name = "DamagePreview"
	damagePreview.Size = UDim2.fromScale(1, 0.3)
	damagePreview.Position = UDim2.fromScale(0, -0.2)
	damagePreview.BackgroundTransparency = 1
	damagePreview.TextColor3 = COLORS.TextDim
	damagePreview.Font = Enum.Font.Gotham
	damagePreview.TextSize = 14
	damagePreview.Text = "Est. Output: ---"
	damagePreview.Parent = inputContainer
	
	local textBox = Instance.new("TextBox")
	textBox.Name = "WordInput"
	textBox.Size = UDim2.fromScale(0.7, 0.8)
	textBox.Position = UDim2.fromScale(0.05, 0.1)
	textBox.BackgroundColor3 = COLORS.LogBg
	textBox.TextColor3 = COLORS.Text
	textBox.Font = Enum.Font.GothamBold
	textBox.TextSize = 18
	textBox.PlaceholderText = "Enter word..."
	textBox.Text = ""
	...
	submitBtn.Size = UDim2.fromScale(0.2, 0.8)
	submitBtn.Position = UDim2.fromScale(0.78, 0.1)
	submitBtn.BackgroundColor3 = COLORS.Player
	...
	local function updatePreview()
		local word = textBox.Text:lower():gsub("%s+", "")
		if word == "" then
			damagePreview.Text = "Est. Output: ---"
			damagePreview.TextColor3 = COLORS.TextDim
			return
		...
		-- Find local player's participant
		local mySlimeName = nil
		for _, p in ipairs(battleData.Participants or {}) do
			if p.PlayerId == Players.LocalPlayer.Name then
				mySlimeName = p.SlimeName
				break
			end
		end
		
		if mySlimeName then
			local entry = SynonymDatabase[mySlimeName:lower()]
			if entry then
				if table.find(entry.Synonyms or {}, word) then
					damagePreview.Text = "Est. Output: 1.5x DAMAGE (Critical!) 💥"
					damagePreview.TextColor3 = COLORS.HPBarLow
				elseif table.find(entry.Antonyms or {}, word) then
					damagePreview.Text = "Est. Output: +50% DEFENSE (Shield!) 🛡️"
					damagePreview.TextColor3 = COLORS.Player
				else
					damagePreview.Text = "Est. Output: 1.0x DAMAGE (Weak) 📉"
					damagePreview.TextColor3 = COLORS.Enemy
				end
			end
		end
	end
	
	textBox:GetPropertyChangedSignal("Text"):Connect(updatePreview)
	...
	local function submitWord()
		if textBox.Text ~= "" then
			...
			BattleService.Client:ExecuteSemanticAction(battleData.BattleId, word):andThen(function(result)
				if result.Success then
					inputPrompt.Text = result.Message
					inputPrompt.TextColor3 = COLORS.Success
				else
					inputPrompt.Text = result.Message
					inputPrompt.TextColor3 = COLORS.Enemy
				end
